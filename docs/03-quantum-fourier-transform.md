...
For an `n`-qubit register, the QFT maps a computational basis state `|x>`
to:

```
QFT|x> = (1/√(2^n)) * Σ_y  exp(2πi·x·y / 2^n) |y>
```
...
2. Apply controlled-phase rotations `CP(π / 2^(n-qubit))` from every qubit
...
This gives an `O(n²)` gate count, an exponential improvement over the
classical FFT's `O(n·2^n)` operations on a state vector of size `2^n` —
...
prepares a counting register whose amplitudes encode a phase `φ ≈ k/r` in
